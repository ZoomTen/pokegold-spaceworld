...
def processGhidra(line):
	...
	# high loads
	line = re.sub('(LDH\s+\()0x([0-9a-fA-F]+)', lambda re: f'{re.group(1)}hFF{re.group(2).upper()}', line)
	line = re.sub('(LDH\s+\()C', lambda re: f'{re.group(1)}c', line)
	line = re.sub('(LDH\s+[ABCDEHL],\s*\()0x([0-9a-fA-F]+)', lambda re: f'{re.group(1)}hFF{re.group(2).upper()}', line)
	...
	# array normalization
	line = re.sub('(\w+)\[(\d+)\]', lambda re: f'{re.group(1)} + {re.group(2)}', line)
	...
	line = re.sub('FUN_rom(\d+)__([0-9a-f]{4})', lambda re: f'Function{to_absolute(re.group(1), re.group(2))}', line)
	line = re.sub('LAB_rom(\d+)__([0-9a-f]{4})', lambda re: f'asm_{to_absolute(re.group(1), re.group(2))}', line)
	line = re.sub('SUB_rom(\d+)__([0-9a-f]{4})', lambda re: f'Function{to_absolute(re.group(1), re.group(2))}', line)
	line = re.sub('(DAT|WORD|BYTE)_rom(\d+)__([0-9a-f]{4})', lambda re: f'data_{to_absolute(re.group(2), re.group(3))}', line)
	line = re.sub('TEXT_rom(\d+)__([0-9a-f]{4})', lambda re: f'text_{to_absolute(re.group(1), re.group(2))}', line)
	line = re.sub('(DAT|WORD|BYTE)_([0-9a-f]{4})', lambda re: f'w{re.group(2)}', line)
	
	line = re.sub('(\w+)\.\.|(\w+)::', lambda re: f'.', line)
